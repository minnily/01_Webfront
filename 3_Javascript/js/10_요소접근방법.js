...
function tagNameTest(){
    //html문서에 존재하는 모든 <li>요소를 얻어와 배열로 묶어서 반환
    const tageList = document.getElementsByTagName("li")
    console.log(tageList);

    for(let i=0; i< tageList.length; i++) { //순차접근
        
        //tageList[i].innerText = "abc" :내용으로 "ABC" 대입하기
        //tageList[i].innerText         :작성된 내용을 얻어오기
        console.log(tageList[i].innerText);

        tageList[i].style.backgroundColor = tageList[i].innerText;
    }
...
function nameTest(){
    ...
    for(let i = 0; i < hobbyList.length; i++){
        ...
        //input요소.checked ->요소가 체크되어 있으면 true, false 반환
        //input요소.checked = true -> 해당 요소를 체크
        //input요소.checked = false -> 해당 요소를 체크해제
