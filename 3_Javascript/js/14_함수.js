...
/* #btn1클릭시 #input1의 값을 읽어와 print1()함수 호출*/
...
document.querySelector("#btn2a").addEventListener("click", function() {
    ...
    //왜 멜론이 아니라 딸기가 출력되는가? (위에 딸기로 작성해 놓은 부분있음)
    //-> arrayTest(arr2) 호출시 arr2 배열을 통째로 복사해서 전달하는 것이 아닌 arr2에
    //저장된 주소만 보내서 함수를 수행함.
    ...
    /* 참조란?
    -객체지향언어에서 배열, 객체 등 object는 값을 여러개 가지고 있는 자료형인데
    변수는 값을 1개만 저장할 수 있기 때문에
    object를 메모리 다른 곳에 저장하고 저장된 곳의 주소를 변수에 저장해서 필요할때마다
    해당주소를 찾아가 원하는 값을 얻어오거나 수정할 수 있게 하는 것
...
document.querySelector("#btn2c").addEventListener("click", function(){
    ...
    const sumFn = function(a,b){
        return a + b; //함수를 끝내고 +값(a+b)을 가지고 호출한 곳으로 돌아감
...
function pow(num,x){
    ...
    let result = 1; //0 (x) 곱했을 때 결과에 영향을 주지 않기 위해 1로 초기화
    for(let i=0; i<x; i++){
        result *= num;
    }
...
function sumFn(a){//전달된 배열 요소의 합을 반환
